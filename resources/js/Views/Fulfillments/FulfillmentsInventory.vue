<template>
    <b-row class="m-0 p-0">
        <b-col cols="12" class="m-0 p-0">
            <b-row class="m-0 p-0">
                <div class="d-sm-block d-md-none py-2 w-100">
                    <b-row>
                        <b-col cols="12">
                            <b-row>
                                <b-col>
                                    <b-button v-b-toggle.collapse-1 variant="info" size="sm" class="float-right"><i
                                        class="fas fa-cog"/></b-button>
                                </b-col>
                            </b-row>
                            <b-row>
                                <b-col>
                                        ...
                                        <b-card>
                                            ...
                                            <b-row>
                                                <b-col class="p-0">
                                                    ...
                                                    >
                                                        <b-form-select id="perPage" :options="pagination.options"
                                                ...
                                                <b-col>
                                                    ...
                                                    >
                                                        <b-pagination id="pagination" :per-page="pagination.perPage"
                ...
                <b-col cols="12" lg="9" class="m-0 p-0 border-left border-right border-1 border-bottom">
                    <div class="table-responsive table-fit border-top border-1" v-if="not_found === false"
                         style="height: 80vh; overflow: scroll; padding-right: 11px;">
                        <b-table
                            id="inventoryTable"
                            striped
                            :sticky-header="true"
                            ...
                            class="border-top-0"
                        >
                            <template slot="thead-top" slot-scope="{ fields, index }">
                                <td :colspan="table.fields.length">
                                    <b-form-group
                                        class="text-left"
                                        id="sku-search"
                                        label="Search by Sky"
                                        label-for="sku-search-input"
                                        description="Enter Sku #"
                                    >
                                        <b-form-input id="sku-search-input"
                                                      placeholder="Enter SKU..."
                                                       v-model="table.filters.sku"/>
                                    </b-form-group>
                                </td>
                            </template>
                            <!-- Loading-->
                            ...
                            <template v-slot:cell(sku)="data">
                                <b-link :to="`/products/sku/${data.item.sku}`">{{ data.item.sku }}</b-link>
                            </template>
                            <template v-slot:head(status)="data">
                                <i class="fas fa-boxes"/>
                            </template>
                            <template v-slot:cell(status)="data">
                                <i class="fas fa-circle stockinfo"
                                   :class="stockAlert(data.item.status).color"
                                   v-b-tooltip.hover
                                   :title="stockAlert(data.item.status).text"/>
                            </template>
                            ...
                            <b-col>
                                <template slot="top-row" slot-scope="{ fields }">
                                    <td v-for="field in fields" :key="field.key">
                                        <input v-model="table.filters[field.key]" :placeholder="field.label">
                                    </td>
                                </template>
                                <b-form-group
                                    id="input-group-1"
                                ...
                                >
                                    <b-button-group class="mx-1 btn-sm pl-0">
                                        ...
                                        <b-button size="sm" @click="exportPDF('selected')"
                                                  v-if="table.selected.length > 0"
                                                  style="max-height: 31px; margin-top: 4px;">Selection
                                        </b-button>
                                    </b-button-group>
                                    <b-button-group class="mx-1 btn-sm pl-0">
                                        <download-csv
        ...
        filtered() {
            if (this.table.data.length > 0) {
                const filtered = this.table.data.filter(item => {
                    return Object.keys(this.table.filters).every(key => {
                        if (key !== 'id' && key !== 'sku') {
                            return item[key] >= this.table.filters[key];
                        } else if (key === 'status') {
                            return item['key'] === this.table.filters[key]
                        } else {
                            return String(item[key]).includes(this.table.filters[key]);
                        }
                    });
                });
                return filtered.length > 0
                    ? filtered
                    : [
                        Object.keys(this.table.data[0]).reduce(function (obj, value) {
                            obj[value] = '';
                            return obj;
                        }, {})
                    ];
        ...
        return {
            ...
            table: {
                ...
                fields: [
                    ...
                    {label: 'Purchase', key: 'purchase_cost'},
                    {label: 'Fee\'s', key: 'fba_fee'},
                    {label: 'FBA Price', key: 'fba_price'},
                    {label: 'Profit', key: 'profit'},
                    {label: 'Inventory', key: 'tpa_wh'},
                    {label: 'FBA Warehouse', key: 'fba_wh'},
                    {label: 'FBA Inbound', key: 'fba_inbound'},
                    {label: 'Turn', key: 'total_turn'}
                ],
                filters: {
                    sku: "",
                    status: "",
    ...
    methods: {
        async exportPDF(type) {
            ...
            autoTable(doc, {
                ...
                columns: [
                    ...
                    {header: 'Avg Cost', dataKey: 'avg_cost'},
                    {header: 'Purchase', dataKey: 'purchase_cost'},
                    {header: 'Fee\'s', dataKey: 'fba_fee'},
                    {header: 'FBA Price', dataKey: 'fba_price'},
                    {header: 'Profit', dataKey: 'profit'},
                    {header: 'Inventory', dataKey: 'tpa_wh'},
                    {header: 'FBA Warehouse', dataKey: 'fba_wh'},
                    {header: 'FBA Inbound', dataKey: 'fba_inbound'},
                    {header: 'Turn', dataKey: 'total_turn'},
                ],
                headStyles: {
                    fillColor: [0, 0, 0],
        ...
        stockAlert(items) {
            switch (items) {
                ...
                case 'low-stock':
                    return {
                        color: 'text-warning',
                        text: 'Low Inventory / Re-Order'
                    };
                case 'in-stock':
                    return {
                        color: 'text-success',
                        text: 'In-Stock / Re-Order'
                    };
                default:
                    return {
                        color: 'text-info',
        ...
        async getProducts() {
            try {
                ...
                this.pagination.totalItems = this.pagination.perPage = parseInt(this.table.data.length);
                this.pagination.options.push(parseInt(this.table.data.length));
            } catch (err) {
                console.log(err);
            } finally {
                this.isBusy = false;
        ...
        async fetchProducts(search) {
            let term = search || '';
            try {
                let response = await (api.get(`/amazon/mws/inventory/today`));
        ...
        formatResponse(response) {
            return response.data.inventory.map(record => {
                record.inventory.map(wh => {
                    let warehouse = 'tampa';
                    if (wh.warehouse.id === 2) {
                        warehouse = 'vegas'
                    }

                    if (wh.warehouse.id === 3) {
                        warehouse = 'fba'
                    }

                    return record[warehouse] = wh.quantity.toString();
                });
                ...
                record.total = (parseInt(record.tampa) || 0) + (parseInt(record.vegas) || 0) + (parseInt(record.fba) || 0);
                record.status = record.total > 100 ? 'overstocked' : record.total < 100 && record.total > 50 ? 'in-stock' : record.total < 50 && record.total > 0 ? 'low-stock' : 'out-of-stock';
                return record;
            });
        }
        ,
        ...
        async fetchResults(val) {
            try {
                this.table.data = await this.fetchProducts(val.query);
            } catch (err) {
                console.log(err);
            } finally {
                this.isBusy = false;
