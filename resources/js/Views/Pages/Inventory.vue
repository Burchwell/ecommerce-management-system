<template>
    <b-row class="m-0 p-0">
        <b-col cols="12" class="m-0 p-0">
            <b-row class="m-0 p-0">
                <b-col cols="12" xl="9" class="m-0 p-0 border-left border-right border-1 border-bottom">
                    <div class="table-responsive table-fit border-top border-1" v-if="not_found === false"
                        ...
                        <b-table
                            ...
                            class="border-top-0"
                        >
                            <template slot="top-row" slot-scope="{ fields, index }">
                                <td v-for="(field, i) in fields" :key="field.key">
                                    <b-form-checkbox v-model="selectAll" @click="selectAll"
                                                     v-if="field.key === 'index'"/>
                                    <template
                                        v-else-if="field.key !== 'index' && field.key !== 'sku' && field.key !== 'status'">
                                        <b-form-input size="sm" v-model="table.filters[field.key]"
                                                      :placeholder="field.label" type="number"
                                                      :min="Math.min.apply(Math, table.data.map(function(o) { return o[field.key]; }))"
                                                      :max="Math.max.apply(Math, table.data.map(function(o) { return o[field.key]; }))"
                                                      step="1"/>
                                    </template>

                                    <b-form-input size="sm" v-model="table.filters[field.key]"
                                                  :placeholder="field.label" v-if="field.key === 'sku'"/>
                                </td>
                            </template>
                            <!-- Loading-->
                ...
                <b-col xl="3">
                        ...
                        <b-col>
                            ...
                            >
                                <b-button-group class="mx-1 btn-sm pl-0">
                                    <b-button size="sm" variant="secondary" style="max-height: 31px; margin-top: 4px;"
                                              @click="exportPDF">Download PDF
                                    </b-button>
                                    <b-button size="sm" @click="exportPDF('selected')" v-if="table.selected.length > 0"
                    ...
                    <b-row>
                        <b-col>
                            ...
                            >
                                <b-form-select id="perPage" :options="pagination.options" v-model="pagination.perPage"/>
                            </b-form-group>
                    ...
                    <b-row>
                        <b-col>
                            ...
                            >
                                <b-pagination id="pagination" :per-page="pagination.perPage" v-model="pagination.currentPage"
...
export default {
    ...
    methods: {
        ...
        async fetchProducts(search) {
            let term = search || '';
            try {
                let response = await (api.get(`/inventory/${term}`));
