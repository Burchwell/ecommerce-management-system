<template>
    <b-card>
        <b-card-header>
            ...
            <div class="d-block">
                <b-dropdown split
                            v-b-popover.top title="Download PDF" v-b-hover
                            split-variant="outline-info" variant="secondary"
                            @click="exportPDF" class="mr-2">
                    <template v-slot:button-content>
                        <i class="fas fa-file-pdf"/>
                    </template>
                    <b-dropdown-item v-if="table.selected.length > 0"
                                     @click="exportPDF('selected');">Export Selected
                    </b-dropdown-item>
                </b-dropdown>
                <b-dropdown split split-variant="info"
                            v-b-popover.top title="Download CSV" v-b-hover
                            variant="secondary" @click="exportCSV">
                    <template v-slot:button-content class="mr-2">
        ...
        <b-card-body>
            ...
            <div class="tableFixHead">
                ...
                >
                    ...
                    <template v-slot:head(order_number)="data">
                        Order Number
                    </template>
                    <template v-slot:cell(order_number)="data">
                        <b-link class="text-info" :to="`/orders/no/${data.item.order_number}`">
                            {{ data.item.order_number }}
                        </b-link>
                    </template>
                    <template v-slot:head(computer_name)="data">
                        Computer
                    </template>
                    <template v-slot:head(created_at)="data" class="text-right">
                        <i class="fas fa-cog"/>
                    </template>
                    <template v-slot:cell(created_at)="data" class="text-right">
                        <b-button-group>
                            <b-button type="button" variant="info" @click="downloadPDF(data.index)">
...
export default {
    ...
    watch: {
        search: function (val) {
            this.pagination = Object.assign(this.pagination, {
                ...
                sortBy: this.pagination.sortBy || 'updated_at',
    ...
    data() {
        return {
            ...
            table: {
                columns: [
                    {key: 'id', label: "", class: 'd-none d-md-table-cell bg-gray-50 text-center', sortable: true},
                    {key: 'order_number', label: "", class: 'd-none d-md-table-cell bg-gray-50', sortable: true},
                    {key: 'computer_name', label: "", class: 'd-none d-md-table-cell bg-gray-50', sortable: true},
    ...
    methods: {
        ...
        async fetchData() {
            try {
                ...
                const response = await api.get(`logs/shippinglabels${filters.length > 0 ? '?' + filters : ''}`);
                this.table.data = response.data.labels.data
                this.pagination = response.data.labels.pagination
            } catch (err) {
                console.log(err);
            } finally {
                this.isBusy = false;
        ...
        async exportPDF(type) {
            ...
            autoTable(doc, {
                ...
                columns: [
                    {header: '#', dataKey: 'id'},
                    {header: 'Order #', dataKey: 'order_number'},
                    {header: 'Computer', dataKey: 'computer_name'},
                    {header: 'Printed At', dataKey: 'printed_at'},
        ...
        downloadPDF(row) {
            window.open(`/api/v1/logs/shippinglabels/pdf/${this.table.data[row].id}?api_token=${localStorage.getItem('access_token')}`);
        },
        exportCSV(type) {
            let columns = [
                {header: '#', dataKey: 'id'},
