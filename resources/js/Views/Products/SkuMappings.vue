<template>
    <div class="mt-4">
        <div class="jumbotron p-3 bg-white" data-pages="parallax">
            <div class="row">
                <div class="col-xl-7 col-lg-6 h-100">
                    <h3 class="pt-4 m-0">Product Sku Mappings</h3>
                </div>
                <div class="col-xl-5 col-lg-6 py-3">
                    <div class="float-right">
                        <b-button type="button" variant="link" size="lg" @click="$router.push('/products')">Go back to
                            Products
                        </b-button>
                    </div>
                </div>
        ...
        <b-card>
            ...
            <b-card-body>
                <div class="tableFixHead">
                    <b-table class="table-striped" hover
                    ...
                    >
                        ...
                        <template v-slot:cell(sku)="data">
                            <b-link :to="`/product/${data.item.sku}`">{{ data.item.sku }}</b-link>
                        </template>
                        <template v-slot:head(skumappings)="data">
                            <span>{{ data.label }}</span>
                        </template>
                        <template v-slot:cell(skumappings)="data">
                            {{data.item.skumappings.map(mapng => mapng.mapping).join(', ')}}
                        </template>
                        <template v-slot:cell(options)="data">
                            <b-button-group>
                                <b-button variant="secondary" size="sm" @click="toggleSkuMappingsModal(data.index)">
                                    <em class="fal fa-pen"/>
                                </b-button>
                            </b-button-group>
                        </template>
                ...
                <SkuMappingsModal
                    ...
                    @skuMappingSaved="updateSkuMappings"
                    @skuMappingDeleted="deleteSkuMapping"
...
export default {
    name: "SkuMappings",
    components: {SkuMappingsModal, Pagination},
    computed: {
        visibleColumns() {
    ...
    data() {
        return {
            ...
            table: {
                columns: [
                    {key: 'index', label: "", class: 'd-none d-md-table-cell bg-gray-50'},
                    {key: 'id', label: "ID", class: 'd-none d-md-table-cell bg-gray-50', sortable: true},
                    {key: 'sku', label: "Sku", class: 'w-sm-20', sortable: true},
                    {key: 'skumappings', label: "Mappings"},
    ...
    methods: {
        ...
        toggleVisibleColumn(index) {
            return this.table.columns[index].visible = !this.table.columns[index].visible
        },
        previousPage() {
        ...
        async fetchData() {
            try {
                ...
                const response = await api.get(`/skumappings${filters.length > 0 ? '?' + filters : ''}`);
