<template>
    <div id="products">
                ...
                <b-row class="m-0 p-0">
                    <b-col cols="12">
                        <div class="widget-11 widget-11-3 card no-border no-margin widget-loader-bar">
                            <b-row>
                                <b-col>
                                    <div class="card-header text-info">Product Fulfillments</div>
                                    <b-row>
                                        <b-col class="px-5">
                                            <!-- PDF -->
                                            <b-form-group label="Downlaod">
                                        ...
                                        <div class="float-right px-3">
                                            ...
                                            <b-button size="small" variant="info"
                                                      v-if="clearable === true"
                                                      @click="resetFilters"
                                            >
                                                Clear
                                    ...
                                    <b-row class="m-0 p-0">
                                        <b-col>
                                            <div
                                                class="widget-11 widget-11-3 card no-border no-margin widget-loader-bar p-3">
                                                <Pagination
                                    ...
                                    <div class="widget-11-3-table tableFixHead p-3">
                                        <b-table class="table-condensed" hover
                                                 :fields="visibleColumns"
                                                 :items="this.table.data"
                                                 :no-local-sorting="true"
                                                 v-on:sort-changed="filterRequest"
                                                 :per-page="pagination.perPage"
                                        ...
                                        >
                                            ...
                                            <template v-slot:head(sku)="data">
                                                <b-form-input size="sm" v-model="pagination.filter"
                                                              placeholder="SKU #" @change="filterRequest"/>
                                            </template>
                                            <template v-slot:cell(sku)="data">
                                                <b-link class="text-info" :to="`/product/${data.item.sku}`">
                                                    {{ data.item.sku }}
                                                </b-link>
                                            </template>
                                            ...
                                            <span v-if="show_ratio" :title="`$ ${data.item.cost}`" class="text-danger"> {{
                                                    data.item.cost_ratio
                                                }} %</span>
                                                <span v-else :title="`${data.item.cost_ratio} %`"
                                                      class="text-danger"> $ {{ data.item.cost }} </span>
                                            </template>
                                            <template v-slot:cell(total_fees_estimate)="data">
                                            <span v-if="show_ratio" :title="`$ ${data.item.total_fees_estimate}`"
                                                  class="text-danger"> {{ data.item.fees_ratio }} %</span>
                                                <span v-else :title="`${data.item.fees_ratio} %`"
                                                      class="text-danger"> $ {{ data.item.total_fees_estimate }} </span>
                                            </template>
                                            <template v-slot:cell(total_profit_estimate)="data">
                                            <span v-if="show_ratio" :title="`$ ${data.item.total_profit_estimate}`"
                                                  :class="profitClass(data.item.profit_ratio)"> {{
                                                    data.item.profit_ratio
                                                }} %</span>
                                                <span v-else :title="`${data.item.profit_ratio} %`"
                                                      :class="profitClass(data.item.profit_ratio)"> $ {{
                                                        data.item.total_profit_estimate
                                                    }} </span>
                                            </template>


                                            <!-- On Hand-->
                                            <template v-slot:cell(quantity_on_hand_tampa)="data">
                                            <span
                                                :class="data.item.quantity_on_hand_tampa <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.quantity_on_hand_tampa
                                                }}</span>
                                            </template>
                                            <template v-slot:cell(quantity_on_hand_las_vegas)="data">
                                            <span
                                                :class="data.item.quantity_on_hand_las_vegas <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.quantity_on_hand_las_vegas
                                                }}</span>
                                            </template>

                                            <template v-slot:head(mobile_iv)="data">
                                                T |
                                                L
                                            </template>
                                            ...
                                            <!-- Turn-->
                                            <template v-slot:cell(turn_around_time_fba)="data">
                                            <span
                                                :class="data.item.turn_around_time_fba <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.turn_around_time_fba
                                                }}</span>
                                            </template>
                                            <template v-slot:cell(turn_around_time_tampa)="data">
                                            <span
                                                :class="data.item.turn_around_time_tampa <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.turn_around_time_tampa
                                                }}</span>
                                            </template>
                                            <template v-slot:cell(turn_around_time_las_vegas)="data">
                                            <span
                                                :class="data.item.turn_around_time_las_vegas <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.turn_around_time_las_vegas
                                                }}</span>
                                            </template>

                                            <template v-slot:cell(turn_around_time_total)="data">
                                            <span
                                                :class="data.item.turn_around_time_total <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.turn_around_time_total
                                                }}</span>
                                            </template>
                                            ...
                                            <template v-slot:cell(ta_mobile)="data">
                                            <span
                                                :class="data.item.ta_fba <= 0 ? 'text-danger' : ''"> {{
                                                    data.item.ta_fba
                                                }}</span>
                                                |
                                                <span
                                                    :class="data.item.ta_tampa <= 0 ? 'text-danger' : ''"> {{
                                                        data.item.ta_tampa
                                                    }}</span>
                                                |
                                                <span
                                                    :class="data.item.ta_vegas <= 0 ? 'text-danger' : ''"> {{
                                                        data.item.total_turn
                                                    }}</span>
                                                |
                                                <span
                                                    :class="data.item.total_turn <= 0 ? 'text-danger' : ''"> {{
                                                        data.item.total_turn
                                                    }}</span>
                                            </template>
                                            ...
                                            <template v-slot:cell(mobile_iv)="data">
                                                {{ data.item['ta_tampa'] }} | {{ data.item['ta_vegas'] }}
                                            </template>
                                            <template v-slot:head(options)="data">
                                                <em class="fad fa-cog"/>
                                            </template>
                                            <template v-slot:cell(options)="data">
                                                <b-button
                                                    ...
                                                    variant="default"
                                                    class="float-right"
                                                    @click="toggleSkuMappingsModal(data.index)"
                                                >
                                                    <em class="fad fa-layer-group"/>
                                                </b-button>
                                            </template>
                                        </b-table>
                                    </div>
        ...
        <SkuMappingsModal
            :skuMapping="skuMappings.data"
            :product="skuMappings.product"
            :show="skuMappingsModalShow"
            @skuMappingsSaved="updateSkuMappings"
            @skuMappingsDeleted="deleteSkuMappings"
            @close="skuMappingsModalShow = false"
        />
    </div>
</template>
<script>
...
import SkuMappingsModal from "./SkuMappingsModal";

export default {
    name: "Products",
    components: {SkuMappingsModal, Filters, Pagination},
    computed: {
        visibleColumns() {
        ...
        clearable() {
            if (this.pagination.filter && this.pagination.filter !== null || this.pagination.sortBy && this.pagination.sortBy !== '' || this.table.selected.length > 0) {
                return true;
            }
            return false;
        },
        ...
        skuMappingsSelectAll: {
            get() {
                return this.skuMappings.data ? this.skuMappings.selected.length === this.skuMappings.data.length : false;
            },
            set(value) {
                var selected = [];
                if (value) {
                    this.skuMappings.data.forEach(function (product) {
                        selected.push(JSON.stringify(product));
                    });
                }
                this.skuMappings.selected = selected;
            }
        }
    },
    watch: {
    ...
    data() {
        return {
            skuMappingsModalShow: false,
            editSkuMappings: false,
            skuMappings: {
                product: {},
                columns: [
            ...
            table: {
                columns: [
                    ...
                    {
                        key: 'channel_advisor_id',
                        label: "ChannelAdvisor ID",
                        class: 'd-none d-md-table-cell bg-gray-50',
                        sortable: true,
                    ...
                    {key: 'weight', label: "Weight (lb)", class: 'text-info w-sm-20', sortable: true},
                    {key: 'length', label: "Length (in)", class: 'text-info w-sm-20', sortable: true},
                    {key: 'width', label: "Width (in)", class: 'text-info w-sm-20', sortable: true},
                    {key: 'height', label: "Height (in)", class: 'text-info w-sm-20', sortable: true},
                    {key: 'total_turn', label: "Total Turn", class: 'text-info w-sm-20', sortable: true},
                    {key: 'options', label: "", class: 'text-center text-info w-sm-20'},
                ],
                data: [],
                selected: [],
            },
    ...
    methods: {
        newSkuMapping() {
            this.skuMappings.data.push({mapping: '', source: ''})
        },
        updateSkuMappings(skuMappings) {
            this.skuMappings.data = skuMappings
            this.skuMappingsModalShow = false;
            this.$bvModal.hide('productSkuMappings')
        },
        deleteSkuMappings(index) {
            this.skuMappings.data.splice(index, 1);
            this.skuMappingsModalShow = false;
            this.$bvModal.hide('productSkuMappings')
        },
        async toggleSkuMappingsModal(index) {
            try {
                this.skuMappings.product = this.table.data[index]
                const response = await api.get(`/products/${this.skuMappings.product.id}/mappings`);
                this.skuMappings.data = response.data.mappings;
                this.skuMappingsModalShow = !this.skuMappingsModalShow;
                this.$bvModal.show('productSkuMappings');
            } catch (error) {
        ...
        filterRequest(e) {
            if (typeof e !== 'string') {
                delete e.apiUrl;
                delete e.filter;
                const formated = JSON.parse(JSON.stringify(e));
                this.pagination = Object.assign(this.pagination, formated);
            }
            this.fetchData();
        },
        async fetchData() {
            try {
                this.isBusy = true;
                let filters = Object.keys(this.pagination).map(k => k !== 'options' && isNaN(k) ? k + '=' + this.pagination[k] : '').filter(value => value.length !== 0).join('&');
                const response = await api.get(`/products${filters.length > 0 ? '?' + filters : ''}`);
                this.table.data = response.data.products.data
        ...
        updatePagination(value) {
            Object.assign(this.pagination, value);
        },
            ...
            autoTable(doc, {
                head: [
                    [
                        ...
                        {content: 'Profitability', colSpan: 3, styles: {fillColor: [199, 28, 34]}},
                        {content: 'Profit', colSpan: 1, styles: {fillColor: [115, 168, 57]}},
                        {content: 'Quantity On Hand', colSpan: 2, styles: {fillColor: [23, 162, 184]}},
                        {content: 'FBA Inventory', colSpan: 4, styles: {fillColor: [47, 164, 231]}},
                        {content: 'Turn', colSpan: 4, styles: {fillColor: [3, 60, 115]}}
                    ],
                    ["#", "SKU", "Price", 'Cost', 'Fee\'s', 'Profit', 'Tampa', 'Las Vegas', 'In-Stock', 'Processing', 'Inbound', 'Pending', 'FBA', 'Tampa', 'Las Vegas', 'Total']
                ],
                body: data,
                columns: [
                    {header: '#', dataKey: 'id'},
                    ...
                    {header: 'Fees', dataKey: 'total_fees_estimate'},
                    {header: 'Profit', dataKey: 'total_profit_estimate'},
                    {header: 'Tampa', dataKey: 'quantity_on_hand_tampa'},
                    {header: 'Las Vegas', dataKey: 'quantity_on_hand_las_vegas'},
                    {header: 'In-Stock', dataKey: 'afn_fulfillable_qty'},
                    {header: 'Processing', dataKey: 'fba_reserved_fc_processing'},
                    {header: 'Inbound', dataKey: 'afn_inbound_shipped_qty'},
                    {header: 'Pending', dataKey: 'fba_reserved_customer_orders'},
                    {header: 'FBA', dataKey: 'turn_around_time_fba'},
                    {header: 'Tampa', dataKey: 'turn_around_time_tampa'},
                    {header: 'Las Vegas', dataKey: 'turn_around_time_las_vegas'},
                    {header: 'Total', dataKey: 'turn_around_time_total'},
                ],
                headStyles: {
                    fillColor: [0, 0, 0],
                ...
                didParseCell: function (row) {
                    ...
                    if (row.section !== 'head') {
                        // if (row.cell && (row.cell.raw === null || row.cell.raw.length === 0)) {
                        //     row.cell.raw = 0;
                        //     row.cell.text = ['0']
                        // }
                        if (row.column.dataKey === 'cost' || row.column.dataKey === 'total_fees_estimate') {
                            row.cell.styles.textColor = [199, 28, 34]
                        } else if (row.column.dataKey === 'total_profit_estimate') {
                            let ratio = ((row.row.cells.total_profit_estimate.raw / row.row.cells.price.raw) * 100);
                            switch (true) {
                                case ratio <= 0:
                                    row.cell.styles.textColor = [199, 28, 34];
                                    break;
                                case ratio > 0 && ratio < 30:
                                    row.cell.styles.textColor = [221, 86, 0];
                                    break;
                                case ratio > 50:
                                    row.cell.styles.textColor = [63, 102, 20];
                                    break;
                                default:
                                    row.cell.styles.textColor = [115, 168, 57];
                                    break;
                            }
                        } else {
                            if (row.cell.raw <= 0) {
                ...
                didDrawPage: function () {
                    ...
                    // Header
                    doc.setFontSize(12);
                    var fileTitle = "FBA Inventory Report";
                    doc.text(fileTitle, 14, 35);
                    doc.addImage(logo, 'JPEG', 10, 10, 50, 16);
        ...
        exportCSV(type) {
            ...
            var csv = data.map(function (row) {
                let csvRow = columns.map((fields, index) => {
                    return row[columns[index].dataKey] || 0
                });
