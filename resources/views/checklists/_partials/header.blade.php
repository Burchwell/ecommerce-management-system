...
<script>
    $(function () {
        $('.mgrAccess').on('click', function () {
            var url = $(this).data('href');
            if (prompt("Enter Password") === "Schlenker!1") {
                window.location = url;
            } else {
                alert("Hell Na!!! You have to enter the right password to open this sesame.")
            }
        })
    });
</script>
<nav class="navbar navbar-dark bg-dark">
    <a href="/" class="navbar-brand">
        <img src="{{ secure_asset('images/skar-audio-logo-black.svg') }}" alt="SkarAudio, Inc." class="logo" style="-webkit-filter: invert(1);
   filter: invert(1);"/>
    </a>
    @if (request()->segment(count(request()->segments())) !== 'eodchecklist')
    ...
    <div class="collapse navbar-collapse justify-content-between" id="navbarToggle">
        <ul class="navbar-nav">
            <li class="nav-item">
                <a class="nav-link" href="/warehouse/eodchecklist">Home <span class="sr-only">(current)</span></a>
            </li>
            @if (request()->segment(count(request()->segments())) !== 'eodchecklist')
                @if (request()->segment(count(request()->segments())-1) === 'view')
                    <li class="nav-item">
                        <a class="nav-link" href="/warehouse/eodchecklist/{{request()->segment(count(request()->segments()))}}">View Pending Tasks</a>
                    </li>
                @else
                    <li class="nav-item">
                        <a class="nav-link" id="mgrAccess" href="javascript:;" data-href="/warehouse/eodchecklist/view/{{request()->segment(count(request()->segments()))}}">View Completed Tasks</a>
                    </li>
                @endif
            @endif
        </ul>
